...
const BrowseLibrary = () => {
  ...
  const { books, semanticResults, errorMessage, isLoading } = useSelector(
    (state) => state.book
  );
  ...
  const trimmedQuery = searchQuery.trim();
  const noResults = trimmedQuery !== "" && safeFilteredBooks.length === 0;
        ...
        <div className="books-section">
          ...
          {noResults && (
            <div className="no-results-message">
              {isSemanticMode
                ? `No results found for "${trimmedQuery}". Try describing themes or tropes.`
                : `No books found with title containing "${trimmedQuery}". Try a different search.`}
            </div>
          )}
